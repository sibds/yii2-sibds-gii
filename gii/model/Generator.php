...
class Generator extends \yii\gii\Generator
{
    ...
    public $tableName;
    public $modelClass;
    public $baseClass = 'sibds\components\ActiveRecord';
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            ...
            [['generateLabelsFromComments', 'useSchemaName'], 'boolean'],
            [['enableI18N'], 'boolean'],
    ...
    public function generateRules($table)
    {
        $types = [];
        $lengths = [];
        $columnsNotRequired = ['created_at', 'updated_at', 'created_by', 'updated_by'];
        foreach ($table->columns as $column) {
            if ($column->autoIncrement) {
                continue;
            }
            if (!$column->allowNull && $column->defaultValue === null) {
                if(in_array($column->name, $columnsNotRequired)){
                    continue;
                }
